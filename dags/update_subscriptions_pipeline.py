...
COMMAND = 'python pronto_dashboard/manage.py update_subscriptions --settings=pronto_dashboard.settings.' + ENVIRONMENT_SETTING

default_args = {
    'owner': 'ProntoTools',
    'description': 'Update subscription every 4 hour',
...
with DAG('update_subscriptions_pipeline', default_args=default_args, schedule_interval="0 */4 * * *", catchup=False) as dag:
	...
	t2 = DockerOperator(
        task_id='update_subscription_pipeline',
        auto_remove=True,
        image=IMAGE_NAME,
        api_version='auto',
        command=COMMAND,
