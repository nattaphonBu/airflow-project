...
COMMAND = 'python pronto_dashboard/manage.py sync_checklists --settings=pronto_dashboard.settings.' + ENVIRONMENT_SETTING

default_args = {
    'owner': 'ProntoTools',
    'description': 'sync_checklist every 02.00',
...
with DAG('sync_checklist_pipeline', default_args=default_args, schedule_interval="0 2 * * *", catchup=False) as dag:
    ...
    t2 = DockerOperator(
        task_id='sync_checklist_pipeline',
        auto_remove=True,
        image=IMAGE_NAME,
        api_version='auto',
        command=COMMAND,
