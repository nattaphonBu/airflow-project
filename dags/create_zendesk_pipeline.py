...
COMMAND = 'python pronto_dashboard/manage.py create_zendesk --settings=pronto_dashboard.settings.' + ENVIRONMENT_SETTING

default_args = {
    'owner': 'ProntoTools',
    'description': 'Create zendesk every 10 minutes',
...
with DAG('create_zendesk_pipeline', default_args=default_args, schedule_interval="*/10 * * * *", catchup=False) as dag:
    ...
    t2 = DockerOperator(
        task_id='create_zendesk_pipeline',
        auto_remove=True,
        image=IMAGE_NAME,
        api_version='auto',
        command=COMMAND,
