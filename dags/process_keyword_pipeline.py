...
COMMAND = 'python pronto_dashboard/manage.py process_keywords_from_authoritylabs --settings=pronto_dashboard.settings.' + ENVIRONMENT_SETTING

default_args = {
    'owners': 'ProntoTools',
    'description': 'process keyword run every 30 minute',
...
with DAG('process_keyword_pipeline', default_args=default_args, schedule_interval=SCHEDULE_TIME, catchup=False) as dag:
    ...
    t2 = DockerOperator(
        task_id='process_keyword_pipeline',
        auto_remove=True,
        image=IMAGE_NAME,
        api_version='auto',
        command=COMMAND,
