...
COMMAND = 'python pronto_dashboard/manage.py send_insights_reports_to_sqs -c annually --settings=pronto_dashboard.settings.' + ENVIRONMENT_SETTING

default_args = {
    'owner': 'ProntoTools',
    'description': 'send insight reports to sqs annually',
...
with DAG('send_insights_reports_to_sqs_annually_pipeline', default_args=default_args, schedule_interval='0 03 1 1 *', catchup=False) as dag:
    ...
    t2 = DockerOperator(
        task_id='send_insights_reports_to_sqs_annually_pipeline',
        auto_remove=True,
        image=IMAGE_NAME,
        api_version='auto',
        command=COMMAND,
